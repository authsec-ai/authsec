...
CREATE TABLE IF NOT EXISTS permissions (
    ...
    action VARCHAR(255) NOT NULL DEFAULT '', -- Added action column as it's standard now
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
-- Create unique constraints that handle NULL tenant_id properly
-- For system-wide entries (tenant_id IS NULL), name must be unique
-- For tenant-specific entries (tenant_id IS NOT NULL), (tenant_id, name) must be unique
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        AND constraint_name = 'roles_name_key'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        AND constraint_name = 'scopes_name_key'
    ) THEN
...
-- Note: Simplified permission seeding as resources table is removed
