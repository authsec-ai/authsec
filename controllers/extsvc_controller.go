...
func (ctl *ExternalServiceController) ensureTenantSchema(tenantID string, tenantDB *gorm.DB) error {
	...
	log.Printf("EXTSVC: initializing schema for tenant %s", tenantID)
	if err := migration.MigrateExternalServiceTables(tenantDB); err != nil {
		return fmt.Errorf("failed to migrate external service tables: %w", err)
	}
	if err := migration.EnsureExternalServicePermissions(ctl.globalDB, tenantID); err != nil {
		return fmt.Errorf("failed to setup external-service permissions: %w", err)
	}
