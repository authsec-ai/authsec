...
// RBAC tables live in the master database.
...
// migration 118. Returns nil (and no error) when there is no matching
// Application — callers fall back to the legacy ClientID during transition.
...
// with the policy's allowed permissions, and caps the TTL.
...
// extractDelegationBearerToken pulls the raw Bearer token from the Authorization header.
func extractDelegationBearerToken(c *gin.Context) string {
