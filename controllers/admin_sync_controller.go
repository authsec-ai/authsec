...
type AdminSyncInput struct {
	...
	EntraConfig *EntraIDConfig       `json:"entra_config,omitempty"`       // Direct Entra config
...
type AdminSyncResult struct {
	...
	PreviewUsers []interface{} `json:"preview_users,omitempty"` // Can be ADUser or EntraIDUser
}
...
func (asc *AdminSyncController) SyncEntraAdminUsers(c *gin.Context) {
	...
	entraController := &EntraIDController{}
...
// Returns (created bool, error) where created=true means new user, created=false means updated existing user
func (asc *AdminSyncController) syncEntraUserToMainDB(entraUser EntraIDUser, tenantID uuid.UUID, clientID, projectID *uuid.UUID) (bool, error) {
	db := config.GetDatabase()
	if db == nil {
		return false, fmt.Errorf("database not initialized")
	}
...
func (asc *AdminSyncController) loadStoredEntraConfig(configID, tenantID string) (EntraIDConfig, error) {
	var syncConfig models.SyncConfiguration
