...
func (r *GlobalRepository) GetVerifiedCustomDomainForTenant(workspaceID string) (string, error) {
	var domain string
...
func (r *ClientRepository) GetClientByEmailAndTenant(email, workspaceID, clientID *string) (*sharedmodels.User, error) {
	var userWithJSONMFA appmodels.UserWithJSONMFAMethods
	...
	if err != nil {
		...
		err = r.DB.Scopes(util.WithUsersMFAMethodArray).
			Where("email = ? AND workspace_id = ?", email, workspaceID).First(&userWithJSONMFA).Error
		if err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
...
func (r *ClientRepository) GetClientByEmailAndTenantForLogin(email, workspaceID string) (*sharedmodels.User, error) {
	var userWithJSONMFA appmodels.UserWithJSONMFAMethods
	err := r.DB.Scopes(util.WithUsersMFAMethodArray).
		Where("email = ? AND workspace_id = ? AND provider = ?", email, workspaceID, "local").First(&userWithJSONMFA).Error
	if err != nil {
		return nil, err
	}
...
func (r *ClientRepository) GetClientByEmailTenantAndClient(email, workspaceID, clientID string) (*sharedmodels.User, error) {
	var userWithJSONMFA appmodels.UserWithJSONMFAMethods
	err := r.DB.Scopes(util.WithUsersMFAMethodArray).
		Where("email = ? AND workspace_id = ? AND client_id = ?", email, workspaceID, clientID).First(&userWithJSONMFA).Error
	if err != nil {
		return nil, err
	}
...
func (r *ClientRepository) GetClientForTOTP(email, workspaceID, clientID string) (*sharedmodels.User, error) {
	var userWithJSONMFA appmodels.UserWithJSONMFAMethods
	query := r.DB.Scopes(util.WithUsersMFAMethodArray).Where("email = ? AND workspace_id = ?", email, workspaceID)
...
func (r *ClientRepository) GetClientForTOTPLogin(email, workspaceID string) (*sharedmodels.User, error) {
	var userWithJSONMFA appmodels.UserWithJSONMFAMethods
	...
	err := r.DB.Scopes(util.WithUsersMFAMethodArray).
		Where("email = ? AND workspace_id = ? AND provider = ? AND mfa_method @> ARRAY[?]::text[]",
			email, workspaceID, "local", "totp").First(&userWithJSONMFA).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// Fallback to find user without TOTP for setup
			err = r.DB.Scopes(util.WithUsersMFAMethodArray).
				Where("email = ? AND workspace_id = ? AND provider = ?",
					email, workspaceID, "local").First(&userWithJSONMFA).Error
			if err != nil {
				return nil, err
			}
		} else {
