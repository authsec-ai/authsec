...
// Stored before redirecting to Hydra, recovered in hmgr via context_id binding.
// TTL ~10 minutes, one-time consumption.
//
// Lifecycle:
//  1. /oauth/authorize creates with State as PK plus a server-generated ContextID
//  2. hmgr GetLoginPageDataHandler parses context_id from Hydra's request_url,
//     calls BindByContextID (idempotent: re-reads if already bound)
...
//  - No fallback to heuristic binding — fail closed
...
type AuthRequestContext struct {
	...
	LoginChallenge   string    `json:"login_challenge" gorm:"type:varchar(255);uniqueIndex"`
