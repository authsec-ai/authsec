...
// ExternalPrincipalAccount is the account an external principal names, when
// its kind states one (D-87: aws_account and aws_principal only).
func ExternalPrincipalAccount(kind, subject string) string {
	switch kind {
	case models.ExternalPrincipalAWSAccount:
...
func (l *claimLoader) externalEndpoint(kind, subject string) evEndpoint {
	acct := ExternalPrincipalAccount(kind, subject)
	return evEndpoint{account: acct, connected: acct == "" || l.accts.Connected(acct)}
}
