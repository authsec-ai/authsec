...
> **What the script does:** checks prerequisites → asks deployment type (local/production) → auto-generates all JWT/encryption/Hydra secrets → writes `.env` → pulls images → starts services → waits for health → triggers the master DB migration. After that, open the UI to complete initial setup and create your admin account.
...
> **Note:** On first startup AuthSec automatically applies master DB migrations. No manual migration step is needed — the setup script also triggers the migration endpoint as a final step.
...
# Apply any new master DB migrations
