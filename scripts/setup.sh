...
echo -e "  ${DIM}This can take 60–90 seconds on first run (DB migrations run automatically on startup)…${NC}\n"
...
# ── Run master DB migrations ──────────────────────────────────────────────────
section "Running database migrations"
info "Triggering master DB migration via API…"
MIG_URL="http://localhost/authsec/migration/migrations/master/run"
MIG_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$MIG_URL" 2>/dev/null || echo "000")
if [[ "$MIG_STATUS" == "200" ]]; then
  ok "Master DB migrations applied"
elif [[ "$MIG_STATUS" == "404" ]]; then
  ok "No pending migrations (schema already up to date)"
else
  warn "Migration endpoint returned HTTP ${MIG_STATUS}."
  warn "If this is a fresh install you may need to run:"
  warn "  curl -X POST http://localhost/authsec/migration/migrations/master/run"
fi
