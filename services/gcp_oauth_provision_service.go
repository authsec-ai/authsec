...
func (s *GCPOAuthProvisionService) Provision(ctx context.Context, sessionID string, callerWorkspaceID uuid.UUID, scope GoogleScope, displayName string, hints *models.GCPOnboardingHints, actor string) (*models.CloudConnector, bool, error) {
	...
	if len(missing) > 0 {
		log.Printf("[gcp] Provision aborted, missing permissions (scopeKind=%s scopeID=%s readerProject=%s): %s",
...
func (s *GCPOAuthProvisionService) onboardWithVerificationRetry(ctx context.Context, workspaceID uuid.UUID, in GCPOnboardInput, actor string) (*models.CloudConnector, bool, error) {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		log.Printf("[gcp] Onboard verification attempt %d/%d not yet accepted by GCP, retrying: %v",
