...
func (m *discoveryManager) ReportSighting(workspaceID uuid.UUID, reportedBy string, in SightingInput) (*models.DiscoveredAgent, bool, error) {
	...
	// Runtime state follows the EVIDENCE. An observed sighting is direct proof
	// the workload exists, so it is running as of observedAt and that is also
	...
	if evidence == models.EvidenceDeclared {
	...
	//
	// For an observed sighting RuntimeStatus is running: it is direct evidence
	// the workload exists, applied by the upsert under a monotonic guard on
	// RuntimeObservedAt so a delayed sighting cannot resurrect an agent deleted
	// after it was observed. A declared sighting leaves runtime unknown.
