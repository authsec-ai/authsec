...
func extractTerraform(v EffectiveVocabulary, filePath string, body []byte) (map[string]interface{}, []string, error) {
	text := string(body)
...
func extractKubernetesManifest(v EffectiveVocabulary, filePath string, body []byte) (map[string]interface{}, []string, error) {
	text := string(body)
...
func extractHelmValues(v EffectiveVocabulary, filePath string, body []byte) (map[string]interface{}, []string, error) {
	text := string(body)
	frameworks := containsAny(text, v.FrameworkTokens)
	if len(frameworks) == 0 {
		return nil, nil, nil
...
func extractSecretReference(v EffectiveVocabulary, filePath string, body []byte) (map[string]interface{}, []string, error) {
	names := collectSecretNames(v.SecretSuffixes, string(body))
	if len(names) == 0 {
		return nil, nil, nil
	}
...
func extractReusableWorkflow(v EffectiveVocabulary, filePath string, body []byte) (map[string]interface{}, []string, error) {
	text := string(body)
...
func extractSelfHostedRunner(v EffectiveVocabulary, filePath string, body []byte) (map[string]interface{}, []string, error) {
